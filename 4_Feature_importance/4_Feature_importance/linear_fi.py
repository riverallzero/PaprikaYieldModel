...
def main():
    ...
    for file in filelist:
        ...
        fi_3 = df_3.groupby(df_3["class"])["coef*std"].sum()
        dataset3.append({
            "Cultivar": cul,
    ...
    plt.subplot(1, 3, 1)
    # 1. 환경, 생육 데이터
    plt.bar(labels, dataset["Environment"], label='환경데이터', color="orange")
    plt.bar(labels, dataset["Growth"], bottom=dataset["Environment"], label='생육데이터', color="yellowgreen")
    ...
    plt.tight_layout()
    plt.savefig("Linear_feature.png", dpi=600)
    plt.show()
